...
export default function Details() {
  ...
  const {data} = useQuery({
    ...
    return (       
    <div className='flex flex-row w-screen items-center mt-72 flex-wrap justify-center gap-10 bg-newblue-950 px-4 md:px-20 pb-20'>
      <Link href='/'>
        <button className='text-newblue-950 flex flex-row bg-yellowneon-500 p-2 rounded-lg justify-evenly w-24 cursor-pointer'>
      ...
      {data &&
        <div key={data.idDrink} className='w-full h-full text-white'>
          <div className='flex flex-col gap-y-4'>
            ...
            <h1 className="md:text-3xl text-2xl text-center  py-4 underline underline-offset-8 decoration-blueneon-500 font-bold">{data.strDrink}</h1>
            <div className='flex flex-col text-newfuchsia-700 md:text-lg text-lg gap-y-4'>
              <p className="md:text-lg text-md text-start font-bold text-blueneon-500">{data.strAlcoholic}</p>
